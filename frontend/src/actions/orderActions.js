...
export const getOrderDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const config = {
      //get request doesn't need to give Content-Type
      headers: {
        Authorization: `Bearer ${userInfo.token}`,
      },
    };
...
export const payOrder = (orderId, details) => async (dispatch, getState) => {
  try {
    dispatch({
...
export const getECPayment = (orderId) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(
      `/api/orders/ecpay/${orderId}/payment`,
      {},
      config
    );
...
export const getLineRequest = (orderId) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(
      `/api/orders/linepay/${orderId}/request`,
      {},
      config
    );
    dispatch({
...
export const getLineConfirm =
  (transactionId, orderId) => async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.get(
        `/api/orders/linepay/confirm?transactionId=${transactionId}&orderId=${orderId}`,
        config
      );
