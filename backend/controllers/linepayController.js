...
function __encrypt(method, api, body, nonce) {
  ...
  let encrypt = null;
  if (method === "GET") {
    encrypt = crypto.HmacSHA256(
      channelSecret + api + body + nonce,
      channelSecret
    );
  }
  if (method === "POST") {
    encrypt = crypto.HmacSHA256(
      channelSecret + api + JSON.stringify(body) + nonce,
      channelSecret
    );
  }
...
//@rout /api/orders/linepay/:id/request
//@access Private
...
export { getLineRequest, getLineConfirm };
