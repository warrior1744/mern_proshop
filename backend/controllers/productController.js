...
const getProducts = asyncHandler(async (req, res) => {
  ...
  const products = await Product.find({ ...keyword })
    .limit(pageSize) //limit the items to show in each page
    .skip(pageSize * (page - 1)); //jump to items base on the page number, if page is 1, then skip none and so on
...
const createProductReivew = asyncHandler(async (req, res) => {
  ...
  if (products) {
    ...
    products.rating =
      products.reviews.reduce((acc, review) => review.rating + acc, 0) /
